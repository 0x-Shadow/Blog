...
- **Terminal playground**: working command line (`help`, `projects`, `notes`, `whoami`, `ascii`, `hire`, `rss`, `stats`, `theme`, `sudo`, `clear`) with ArrowUp/Down history.
- **CrewTrack spotlight**: featured build card (▲80 · r/esp32, spec chips, SOURCE link).
- **TOP_3** project preview + **LATEST** notes preview.
...
### 4. `blog.html` — Notes index
- Search filter + RSS button, cards linking to full posts (date · tag · read time).

### 5. `post.html?id=` — Full note
...
- **Content system**: `POSTS` (notes, newest first), `RELATED_POST` (repo→note links), `PROJECT_SHOTS` (repo→banner). HOW-TO-POST comment included: new note = 30 seconds.
